...
function removeGeometry() {
    const mesh = scene.getObjectByName("mesh1");
    if (mesh) {
        transControls.detach();  // Gỡ bỏ đối tượng khỏi TransformControls trước khi xóa
        scene.remove(mesh);
        gui.removeFolder(objectFolder);
        RemoveAllAnimation();
...
function setLight(LightID) {
    ...
    switch (LightID) {
        ...
        case 3: // Directional light
            ...
            dlFolder.add(dlSettings, 'translate').onChange((value) => {
                if (value) {
